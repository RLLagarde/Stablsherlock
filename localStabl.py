...
class Stabl(SelectorMixin, BaseEstimator):
    ...
    def _generate_artificial_batch(self, X, rng):
        ...
        for i in range(n_real):
            if self.artificial_type == "random_permutation":
                X_art[:, i] = rng.permutation(X[:, i])
            elif self.artificial_type == "knockoff":
    ...
    def _one_bootstrap(self, X, y, subsample_idx, lambda_val, corr_groups, rng):
        ...
        X_full = np.concatenate([X_bs, X_art[mask]], axis=1)
...
#######HERE WE HAVE COMMENTED THE PART WHERE X WAS CHANGED INTO X + NOISE######
        ...
        for idx, lambda_val in tqdm(
            ...
            selected_variables = Parallel(n_jobs=self.n_jobs)(
                delayed(self._one_bootstrap)(
                    ...
                    rng=np.random.RandomState(int(seeds[bs%5, idx])) #this one gives one noise per bts/per lambda
                )
